		...
		switch(pick(safety))//Chance based on the safety list.
			...
			if(2)
				switch(rand(1,3))//Food, drinks, or things. Only selectable once.
					if(1)
						. += rlowertext(pick(drinks))
					if(2)
						. += rlowertext(pick(foods))
					if(3)
						. += rlowertext(pick(locations))
				safety -= 2
			if(3)
				switch(rand(1,4))//Abstract nouns, objects, adjectives, threats. Can be selected more than once.
					if(1)
						. += rlowertext(pick(nouns))
					if(2)
						. += rlowertext(pick(objects))
					if(3)
						. += rlowertext(pick(adjectives))
					if(4)
						. += rlowertext(pick(threats))
		if(!return_list)
			if(words==1)
				. += "."
			else
